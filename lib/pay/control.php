<?php
	...
	/*  判断是否有可执行的订单  */
	$ordernum=$sql("SELECT count(*) FROM pay_order WHERE goodsid<>'0' AND ispay='1' AND isdo='0' ");
	if($ordernum==0){
	...
	/* 按下单的时间执行 */
	$order = $sql("SELECT * FROM pay_order WHERE goodsid<>'0' AND ispay='1' AND isdo='0' ORDER BY id ASC LIMIT 0,1");
	$goods = $sql("SELECT * FROM pay_goods WHERE id='".$order['goodsid']."' ");
	
	if($goods['dowhat']=='POST对接'){
		$val = json_decode($order['goodsval'],TRUE);
		$domsg = hs($val['get'],$val['post'],$val['cookie']);
		$domsg = mb_substr($domsg, 0, 299);
		
	}else if($goods['dowhat']=="亿乐社区"){
		$conf = json_decode($goods['doconfig'],TRUE);
		$orde = json_decode($order['goodsval'],TRUE);
		if($orde['value2']=="null")$orde['value2']='';
		if($orde['value3']=="null")$orde['value3']='';
		if($orde['value4']=="null")$orde['value4']='';
		if($orde['value5']=="null")$orde['value5']='';
		if($orde['value6']=="null")$orde['value6']='';
		$params = [
		    'api_token' => $conf['tokenid'],
		    'timestamp' => time(),
		    'gid' => $conf['gid'],
		    'num' => $conf['num']*$orde['num'],
		    'value1' => $orde['value1'],
		    'value2' => $orde['value2'],
		    'value3' => $orde['value3'],
		    'value4' => $orde['value4'],
		...
		$params['sign'] = getSign($params, $conf['key']);
		$api = "http://".$conf['api'].'.api.94sq.cn/api/order';
		$domsg = hs($api,$params);
		
	}else if($goods['dowhat']=="用户系统充值余额"){
		$conf = json_decode($goods['doconfig'],TRUE);
		$orde = json_decode($order['goodsval'],TRUE);
		$money = floor($conf['money']*$orde['money']*100)/100;
		$admin = $sql("SELECT * FROM admin WHERE id='".$goods['admin_id']."' ");
		$res = $sql("UPDATE  `users` SET money=money+'{$money}' WHERE user='".$orde['user']."' AND admin_id='".$goods['admin_id']."'");
		$uplog = $sql("INSERT INTO `users_log_money` (`admin_id`,`user`,`num`,`msg`,`addtime`) VALUES ('".$goods['admin_id']."','".$orde['user']."','{$money}','在线充值','{$date}')");
		$domsg = '已完成订单';
		...
		$orde = json_decode($order['goodsval'],TRUE);
		$admin = $sql("SELECT * FROM admin WHERE id='".$goods['admin_id']."' ");
		$users = $sql("SELECT * FROM users WHERE admin_id='".$goods['admin_id']."' AND user='".$orde['user']."' ");
		$num = $orde['num'];
		if(strtotime($users['vip'])>time()){
			$vip = date("Y-m-d H:i:s",(strtotime($users['vip'])+(86400*$num*31)));
		}else{
			$vip = date("Y-m-d H:i:s",(strtotime("now")+(86400*$num*31)));
		}
		$res = $sql("UPDATE  `users` SET vip='{$vip}' WHERE user='".$orde['user']."' AND admin_id='".$goods['admin_id']."'");
		$day = $num*31;
		$uplog = $sql("INSERT INTO `users_log_vip` (`admin_id`,`user`,`num`,`msg`,`addtime`) VALUES ('".$goods['admin_id']."','".$orde['user']."','{$day}','在线充值','{$date}')");
		$domsg = '已完成订单';
	}else if($goods['dowhat']=="自动发货"){
		$orde = json_decode($order['goodsval'],TRUE);
		if($sql("SELECT count(*) FROM pay_goods_km WHERE goodsid='".$order['goodsid']."' AND issell='0' ")<$orde['num']){
			$domsg='库存不足无法发货，请联系管理员补发';
		}else{
			$domsg='';
			for($i=0;$i<$orde['num'];$i++){
				$res = $sql("SELECT * FROM pay_goods_km WHERE goodsid='".$order['goodsid']."' AND issell='0'");
				if($i==0){
					$domsg=$res['km'];
				}else{
					$domsg=$domsg."|".$res['km'];
				}
				$res = $sql("UPDATE  `pay_goods_km` SET issell='1',orderid='".$order['id']."' WHERE id='".$res['id']."'");
			}
			if($orde['mail']!=''){
				$admin = $sql("SELECT * FROM admin WHERE id='".$goods['admin_id']."' ");
				...
				if($res['smtp']==''){}
					...
					$who = $orde['mail'];
					$title = "发货通知";
					$text = "您购买的物品为:<br><b>".$domsg."</b><br><br>(若购买多件，则以‘|’分割)";
	...
	$res = $sql("UPDATE  `pay_order` SET isdo='1',do_time='{$date}',do_msg='{$domsg}' WHERE id='".$order['id']."' ");
	$res = $sql("UPDATE  `pay_goods` SET havenum=havenum-'1',sell=sell-'1' WHERE id='".$goods['id']."' ");
	echo "yes";
